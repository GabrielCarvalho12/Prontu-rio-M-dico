...
var Popover = Class.extend(ListenerMixin, {
	...
	position: function() {
		...
		// constrain to the public port. if constrained by two edges, give precedence to top/left
...
var Grid = FC.Grid = Class.extend(ListenerMixin, {
	...
	isRTL: null, // shortcut to the public's isRTL option
	...
	// Renders the basic structure of grid public before any content is rendered
...
Grid.mixin({
	...
	buildBusinessHourEvents: function(wholeDay, businessHours) {
		...
		if (businessHours == null) {
			// fallback
			// access from calendawr. don't access from public. doesn't update with dynamic options.
	...
	// Attaches event-element-related handlers for *all* rendered event segments of the public.
	...
	buildSegDragListener: function(seg) {
		...
		// Tracks mouse movement over the *public's* coordinate map. Allows dragging and dropping between subcomponents
		// of the public.
		var dragListener = this.segDragListener = new HitDragListener(view, {
			...
			interactionStart: function(ev) {
				...
				mouseFollower = new MouseFollower(seg.el, {
					...
					zIndex: 2 // one above the .fc-public
				});
	...
	eventToSpans: function(event) {
		...
		else { // out of public's valid range
			return [];
		}
	},
	...
	// Ensures the given range is within the public's activeRange and is correctly localized.
...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	numbersVisible: false, // should render a row for day/week numbers? set by outside public. TODO: make internal
	...
	// Relies on the public's colCnt and rowCnt. In the future, this component should probably be self-sufficient.
...
DayGrid.mixin({
	...
	renderMoreLink: function(row, col, hiddenSegs) {
		...
		return $('<a class="fc-more"/>')
			...
			.on('click', function(ev) {
				...
				else if (typeof clickOption === 'string') { // a public name
	...
	showSegPopover: function(row, col, moreLink, segs) {
		...
		options = {
			...
			parentEl: this.view.el, // attach to root of public. guarantees outside of scrollbars.
...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	// Relies on the public's colCnt. In the future, this component should probably be self-sufficient.
	renderDates: function() {
	...
	renderSlatRowHtml: function() {
		...
		var slotDate; // will be on the public's first day, but we only care about its time
		var isLabeled;
		var axisHtml;
	...
	renderNowIndicator: function(date) {
		...
		if (segs.length > 0) { // is the current time in public?
...
/* Methods for rendering SEGMENTS, pieces of content that live on the public
...
TimeGrid.mixin({
	...
	// Renders the DOM that the public's content will live in
...
var View = FC.View = Model.extend({
	...
	type: null, // subclass' public name (string)
	name: null, // deprecated. use `type` instead
	title: null, // the text that will be displayed in the header's title

	calendar: null, // owner Calendar object
	viewSpec: null,
	options: null, // hash containing all options. already merged with public-specific-options
	el: null, // the public's containing element. set by Calendar
	...
	// Triggers handlers that are public-related. Modifies args before passing to calendar.
	publiclyTrigger: function(name, thisObj) { // arguments beyond thisObj are passed along
		...
		return calendar.publiclyTrigger.apply(
			...
			[name, thisObj || this].concat(
				...
				[ this ] // always make the last argument a reference to the public. TODO: deprecate
			)
		);
	},
	...
	// Sets the public's title property to the most updated computed value
	...
	// Computes what the title at the top of the calendar should be for this public
	...
	// Generates HTML for an anchor to another public into the calendar.
	// Will either generate an <a> tag or a non-clickable <span> tag, depending on enabled settings.
	// `gotoOptions` can either be a moment input, or an object with the form:
	// { date, type, forceOff }
	// `type` is a public-type like "day" or "week". default value is "day".
	...
	// Sets the container element that the public should render inside of, does global DOM-related initializations,
	...
	// Removes the public's container element from the DOM, clearing any content beforehand.
	...
	// Renders the basic structure of the public before any content is rendered
	renderSkeleton: function() {
		// subclasses should implement
	},


	// Unrenders the basic structure of the public
	...
	// Determing when the "meat" of the public is rendered (aka the base)
	// -----------------------------------------------------------------------------------------------------------------
	...
	// Misc public rendering utils
	// -----------------------------------------------------------------------------------------------------------------


	// Binds DOM handlers to elements that reside outside the public container, such as the document
	bindGlobalHandlers: function() {
	...
	// Unbinds DOM handlers from elements that reside outside the public container
	unbindGlobalHandlers: function() {
		this.stopListeningTo(GlobalEmitter.get());
	},
	...
	// Renders business-hours onto the public. Assumes updateSize has already been called.
	renderBusinessHours: function() {
		// subclasses should implement
	},
	...
	// Immediately unrenders the public's current time indicator and stops any re-rendering timers.
	...
	// if `isAuto` is set to true, height becomes merely a suggestion and the public should use its "natural" height.
	...
	// Renders the events onto the public.
	renderEvents: function(events) {
		// subclasses should implement
	},


	// Removes event elements from the public.
	unrenderEvents: function() {
		// subclasses should implement
	},
	...
	// The `this` value of the callback function will be the public.
	...
	// Retrieves all the rendered segment objects for the public
	getEventSegs: function() {
		// subclasses must implement
		return [];
	},
	...
	// Must be called when an event in the public is dropped onto new location.
	...
	// Must be called when an event in the public has been resized to a new length
	...
	// Selects a date span on the public. `start` and `end` are both Moments.
...
View.mixin({
	...
	// range the public is formally responsible for.
	// for example, a month public might have 1st-31st, excluding padded dates
	currentRange: null,
	...
	// Builds a structure with info about what the dates/ranges will be for the "prev" public.
	...
	// Builds a structure with info about what the dates/ranges will be for the "next" public.
	...
	buildRangeFromDuration: function(date, direction, duration, unit) {
		...
		// if the public displays a single day or smaller
...
var Calendar = FC.Calendar = Class.extend(EmitterMixin, {
	...
	viewsByType: null, // holds all instantiated public instances, current or not
	...
	// Given a public name for a custom public or a standard public, creates a ready-to-go View object
	...
	// Returns a boolean about whether the public is okay to instantiate at some point
	...
	// Forces navigation to a public for the given date.
	// `viewType` can be a specific public name or a generic one like "week" or "day".
	...
	rerenderEvents: function() { // API method. destroys old events if previously rendered.
		if (this.elementVisible()) {
			this.reportEventChange(); // will re-trasmit events to the public, causing a rerender
		}
	}
...
Calendar.mixin({
	...
	dynamicOverrides: null, // options set with dynamic setter method. higher precedence than public overrides.
	...
	setOptions: function(newOptionHash) {
		...
		// catch-all. rerender the header and footer and rebuild/rerender the current public
		this.renderHeader();
		this.renderFooter();
...
Calendar.mixin({
	...
	viewSpecCache: null, // cache of public definitions (initialized in Calendar.js)


	// Gets information about how to create a public. Will use a cache.
	...
	// Given a duration singular unit, like "week" or "day", finds a matching public spec.
	...
	// Builds an object with information on how to create a given public
	buildViewSpec: function(requestedViewType) {
		var viewOverrides = this.overrides.views || {};
		var specChain = []; // for the public. lowest to highest priority
		var defaultsChain = []; // for the public. lowest to highest priority
		var overridesChain = []; // for the public. lowest to highest priority
		var viewType = requestedViewType;
		var spec; // for the public
		var overrides; // for the public
		var durationInput;
		var duration;
		var unit;

		// iterate from the specific public definition to a more general one until we hit an actual View class
		while (viewType) {
			...
			if (overrides) {
				overridesChain.unshift(overrides); // public-specific option hashes have options at zero-level
		...
		if (durationInput) {
			...
			if (duration.valueOf()) { // valid?
				...
				// public is a single-unit duration, like "week" or "day"
	...
	// Builds and assigns a public spec's options object from its already-assigned defaults and overrides
	buildViewSpecOptions: function(spec) {
		spec.options = mergeOptions([ // lowest to highest priority
			Calendar.defaults, // global defaults
			spec.defaults, // public's defaults (from ViewSubclass.defaults)
			this.dirDefaults,
			this.localeDefaults, // locale and dir take precedence over public's defaults!
			this.overrides, // calendar's overrides (options given to constructor)
			spec.overrides, // public's overrides (public-specific options)
	...
	// Computes and assigns a public spec's buttonText-related options
	buildViewSpecButtonText: function(spec, requestedViewType) {

		// given an options object with a possible `buttonText` hash, lookup the buttonText for the
		// requested public, falling back to a generic unit entry like "week" or "day"
		function queryButtonText(options) {
			var buttonText = options.buttonText || {};
			return buttonText[requestedViewType] ||
				// public can decide to look up a certain key
		...
		spec.buttonTextOverride =
			...
			spec.overrides.buttonText; // `buttonText` for public-specific options is a string
		...
		spec.buttonTextDefault =
			...
			requestedViewType; // fall back to given public name
	}
...
Calendar.mixin({
	...
	initialRender: function() {
		...
		this.contentEl = $("<div class='fc-public-container'/>").prependTo(el);
	...
	destroy: function() {
		...
		if (this.view) {
			...
			// NOTE: don't null-out this.public in case API methods are called after destroy.
			// It is still the "current" public, just not rendered.
		}
	...
	// Renders a public because of a date change, public-type change, or for the first time.
	// If not given a viewType, keep the current public but render different dates.
	...
	renderView: function(viewType, forcedScroll) {
		...
		// if viewType is changing, remove the old public's rendering
		if (needsClearView) {
			this.freezeContentHeight(); // prevent a scroll jump when public element is removed
			this.clearView();
		}

		// if viewType changed, or the public was never created, create a fresh public
		if (!this.view && viewType) {
			...
			this.view.setElement(
				$("<div class='fc-public fc-" + viewType + "-public' />").appendTo(this.contentEl)
			);
	...
	// Unrenders the current public and reflects this change in the Header.
	// Unregsiters the `public`, but does not remove from viewByType hash.
	clearView: function() {
	...
	// Destroys the public, including the public object. Then, re-instantiates it and renders it.
	...
	windowResize: function(ev) {
		if (
			...
			this.view.renderRange // public has already been rendered
		) {
...
// Return events objects for business hours within the current public.
// Abuse of our event system :(
Calendar.prototype.getCurrentBusinessHourEvents = function(wholeDay) {
	return this.computeBusinessHourEvents(wholeDay, this.opt('businessHours'));
};

// Given a raw input value from options, return events objects for business hours within the current public.
...
/* An abstract class for the "basic" views, as well as month public. Renders one or more rows of day cells.
----------------------------------------------------------------------------------------------------------------------*/
...
var BasicView = FC.BasicView = View.extend({
	...
	// Generates the DayGrid object this public needs. Draws from this.dayGridClass
	...
	// Renders the public into `this.el`, which should already be assigned
	renderDates: function() {
		...
		this.el.addClass('fc-basic-public').html(this.renderSkeletonHtml());
		this.renderHead();
	...
	// Unrenders the content of the public. Since we haven't separated skeleton rendering from date rendering,
	...
	// Builds the HTML skeleton for the public.
	...
	// Refreshes the horizontal dimensions of the public
	...
	// Adjusts the vertical dimensions of the public to the specified values
	...
	// given a desired total height of the public, returns what the height of the scroller should be
	...
	// Sets the height of just the DayGrid component in this public
	...
	// Renders the given events onto the public and populates the segments array
	renderEvents: function(events) {
	...
	// Retrieves all segment objects that are rendered in the public
	getEventSegs: function() {
...
/* A month public with day cells running in rows (one-per-week) and columns
----------------------------------------------------------------------------------------------------------------------*/
...
var AgendaView = FC.AgendaView = View.extend({
	...
	timeGrid: null, // the main time-grid subcomponent of this public
	...
	initialize: function() {
		...
		if (this.opt('allDaySlot')) { // should we display the "all-day" area?
			this.dayGrid = this.instantiateDayGrid(); // the all-day subcomponent of this public
		}
	...
	// Instantiates the TimeGrid object this public needs. Draws from this.timeGridClass
	...
	// Instantiates the DayGrid object this public might need. Draws from this.dayGridClass
	instantiateDayGrid: function() {
	...
	// Renders the public into `this.el`, which has already been assigned
	renderDates: function() {
		...
		this.el.addClass('fc-agenda-public').html(this.renderSkeletonHtml());
		this.renderHead();
	...
	// Renders events onto the public and populates the View's segment array
	renderEvents: function(events) {
		var dayEvents = [];
		var timedEvents = [];
		var daySegs = [];
...
Its "el" is the inner-content of the above public's scroller.
*/
var ListViewGrid = Grid.extend({
	...
	// render the event segments in the public
